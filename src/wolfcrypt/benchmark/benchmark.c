                                   ...
                                   int countSz, double start, int ret)
{
    ...
    if (csv_format == 1) {
        ...
        XSNPRINTF(msg, sizeof(msg), "%-16s%s %5d %s %s %5d.%03d %s, %8d.%03d %s/s",
        desc, BENCH_ASYNC_GET_NAME(doAsync), (int)blocks, blockType, word[0],
    ...
    const char* desc, int doAsync, int count, double start, int ret)
{
    ...
    if (csv_format == 1) {
        ...
        XSNPRINTF(msg, sizeof(msg), "%-6s %5d %-9s %s %6d %s %5d.%03d %s, %s %5d.%03d ms,"
        " %d.%03d %s\n", algo, strength, desc, BENCH_ASYNC_GET_NAME(doAsync),
        count, word[0], (int)total, (int)((total - (int)total) * 1000), word[1],
        word[2], (int)milliEach, (int)((milliEach - (int)milliEach) * 1000),
        (int)opsSec, (int)((opsSec - (int)opsSec) * 1000), word[3]);
    }
    printf("%s", msg);
...
int benchmark_init(void)
{
    ...
    if (csv_format == 1) {
        printf("wolfCrypt Benchmark (block bytes %d, min %d.%01d sec each)\n",
        (int)BENCH_SIZE, (int)BENCH_MIN_RUNTIME_SEC,
        (int)((BENCH_MIN_RUNTIME_SEC - (int)BENCH_MIN_RUNTIME_SEC) * 1000));
